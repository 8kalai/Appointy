...
const AddDoctor = () => {
  ...
  // 🟢 Accessing backendUrl and aToken from AdminContext
  const { backendUrl } = useContext(AdminContext)
  const { aToken } = useContext(AdminContext)
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      // 🟢 CRITICAL FIX: Sending the Admin Token in the correct 'Authorization' header format
      const response = await axios.post(`${backendUrl}/api/admin/add-doctor`, formData, {
        headers: { 
          'Authorization': `Bearer ${aToken}`, // This is the correct format for JWT
  ...
  return (
    <form onSubmit={onSubmitHandler} className='m-5 w-full'>
      ...
      <div className='bg-white px-8 py-8 border rounded w-full max-w-4xl max-h-[80vh] overflow-y-scroll'>
        <div className='flex items-center gap-4 mb-8 text-gray-500'>
          <label htmlFor="doc-img">
            {/* Using optional chaining and ensuring assets are available */}
            <img 
              className='w-16 bg-gray-100 rounded-full cursor-pointer' 
              src={docImg ? URL.createObjectURL(docImg) : assets.upload_area} 
              alt="Upload Area" 
              onError={(e) => { e.target.src = assets.upload_area; e.target.onerror = null; }}
            />
          </label>
          <input onChange={(e) => setDocImg(e.target.files[0])} type="file" name="doc-img-file" id="doc-img" hidden required />
          <p>Upload doctor <br /> picture</p>
        </div>
