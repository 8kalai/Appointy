...
export const addDoctor = async (req, res) => {
    try {
        ...
        if (exists) {
            fs.unlinkSync(req.file.path);
            return res.status(409).json({ success: false, message: "Doctor email already exists." });
        }
...
// ----------------- Doctor Operations (NO JWT VERSION) -----------------

export const doctorProfile = async (req, res) => {
    try {
        const docId = req.body.doctorId;
        if (!docId) return res.status(400).json({ success: false, message: "doctorId is required" });
