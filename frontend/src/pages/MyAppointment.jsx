...
const MyAppointment = () => {
  ...
  return (
    <div>
      <p className="pb-3 mt-12 text-lg font-medium text-gray-600 border-b">My appointments</p>
      {appointments.length === 0 && <p className="text-gray-500 mt-4">No appointments found</p>}
      {appointments.map((item) => (
        <div key={item._id} className="grid grid-cols-1 gap-4 py-4 border-b">
          <div className="flex flex-col sm:flex-row justify-between">
            ...
            <div className="flex flex-col gap-2 justify-end text-sm text-center mt-2 sm:mt-0">
              {!item.cancelled ? (
                <button className="text-[#696969] sm:min-w-48 py-2 border rounded cursor-pointer">Cancel</button>
              ) : (
                <button className="sm:min-w-48 py-2 border border-red-500 rounded text-red-500">Cancelled</button>
